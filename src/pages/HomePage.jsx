      ...
      <section className="w-full px-3 sm:px-4 md:px-6 lg:px-8 py-8 sm:py-12 md:py-16">
        <div className="max-w-7xl mx-auto">
          <div className="mb-8 sm:mb-10 md:mb-12">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-2 sm:mb-3 md:mb-4" style={{ color: '#000' }}>
              Our Tree Collection
            </h2>
            <p className="text-base sm:text-lg md:text-lg" style={{ color: '#000' }}>
              Explore {filteredTrees.length} amazing trees with detailed information and QR codes
            </p>
          </div>
          ...
          {filteredTrees.length === 0 ? (
            <div className="text-center py-12 sm:py-16 md:py-16">
              <Search className="w-12 h-12 sm:w-14 md:w-16 mx-auto mb-3 sm:mb-4 md:mb-4" style={{ color: '#86CA6B' }} />
              <p className="text-xl sm:text-2xl md:text-2xl" style={{ color: '#000' }}>No trees found matching your search</p>
            </div>
          ) : (
            <div className="relative w-full">
              ...
              <div className="flex gap-3 sm:gap-4 md:gap-6">
                {visibleTrees.map((tree) => (
                  <div
                    key={tree.id}
                    onClick={() => navigate(`/tree/${tree.id}`)}
                    className="flex-1 min-w-0 bg-white rounded-xl sm:rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105 cursor-pointer overflow-hidden border-2"
                    style={{ borderColor: '#86CA6B' }}
                  >
                    {/* Tree Image */}
                    <div className="h-40 sm:h-48 md:h-56 relative overflow-hidden" style={{ backgroundColor: '#86CA6B' }}>
                      <img
                        src={tree.image}
                        alt={tree.nameEnglish}
                        className="w-full h-full object-cover opacity-90 hover:opacity-100 transition-opacity duration-300"
                    ...
                    <div className="p-3 sm:p-4 md:p-6">
                      <h3 className="text-lg sm:text-xl md:text-2xl font-bold mb-1 sm:mb-2 line-clamp-2" style={{ color: '#000' }}>
                        {tree.nameEnglish}
                      </h3>
                      <p className="text-xs sm:text-sm font-semibold mb-2 sm:mb-3" style={{ color: '#86CA6B' }}>
                        {tree.nameKannada}
                      </p>
                      <p className="text-xs text-gray-600 italic mb-3 sm:mb-4 line-clamp-1">
                        {tree.scientificName}
                      </p>
                      ...
                      {/* Button */}
                      <button
                        className="w-full py-2 px-3 sm:px-4 rounded-lg font-semibold transition-all duration-300 text-white cursor-pointer text-sm sm:text-base"
                        style={{ backgroundColor: '#86CA6B' }}
                        onMouseEnter={(e) => e.target.style.backgroundColor = '#000'}
                        onMouseLeave={(e) => e.target.style.backgroundColor = '#86CA6B'}
                      >
              ...
              <button
                onClick={prevSlide}
                className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-8 sm:-translate-x-10 md:-translate-x-12 z-10 p-2 sm:p-3 rounded-full transition-all duration-300 hover:scale-110"
                style={{ backgroundColor: '#86CA6B', color: 'white' }}
                onMouseEnter={(e) => e.target.style.backgroundColor = '#000'}
                onMouseLeave={(e) => e.target.style.backgroundColor = '#86CA6B'}
              >
                <ChevronLeft className="w-5 h-5 sm:w-6 md:w-6 md:h-6" />
              </button>

              <button
                onClick={nextSlide}
                className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-8 sm:translate-x-10 md:translate-x-12 z-10 p-2 sm:p-3 rounded-full transition-all duration-300 hover:scale-110"
                style={{ backgroundColor: '#86CA6B', color: 'white' }}
                onMouseEnter={(e) => e.target.style.backgroundColor = '#000'}
                onMouseLeave={(e) => e.target.style.backgroundColor = '#86CA6B'}
              >
                <ChevronRight className="w-5 h-5 sm:w-6 md:w-6 md:h-6" />
              </button>
